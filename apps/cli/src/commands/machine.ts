...
export function registerMachineCommands(
  ...
  machine
    ...
    .option(
      "--shell <sh|powershell>",
      "Target shell for manual enrollment (default: sh)",
    )
    .option("--json", "Print machine-readable JSON output")
    .action(
      action(async (opts: MachineCreateCommandOptions) => {
        ...
        if (opts.shell !== undefined) {
          if (opts.shell !== "sh" && opts.shell !== "powershell")
            throw new Error("--shell must be sh or powershell.");
          if (machineProviderId !== "manual" || !opts.wait)
            throw new Error(
              "--shell requires --provider manual and cannot be combined with --no-wait.",
            );
        }
            ...
            if (enrollmentCommand !== null) {
              console.error(
                opts.shell === "powershell"
                  ? enrollmentCommand.windowsCommand
