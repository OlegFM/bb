...
describe("machine enroll", () => {
  it("retries a lost exchange with replacement bootstrap while preserving the reserved identity", async () => {
    ...
    await expectPrivateFile(join(h.dir, "auth.json"));
  });
  ...
  it("exchanges through authorization, persists private credentials, and no-ops on same identity with expired material", async () => {
    ...
    await expectPrivateFile(join(h.dir, "auth.json"));
    await expectPrivateFile(join(h.dir, "config.json"));
    await expectPrivateFile(join(h.dir, "host-id"));
    await expectPrivateFile(join(h.dir, "host-daemon-port"));
    const port = Number(
  ...
  it.runIf(process.platform === "win32")(
    "recognizes the private Windows npm launcher without a POSIX bin shim",
    async () => {
      const h = await harness();
      const launcher = join(h.dir, "npm", "bb-app.cmd");
      await mkdir(join(h.dir, "npm"), { recursive: true });
      await writeFile(launcher, "@echo off\r\n");
      await expect(h.run()).resolves.toEqual({ hostId: "host_test" });
      expect(await readFile(launcher, "utf8")).toBe("@echo off\r\n");
      await expect(
        stat(join(h.dir, "npm", "bin", "bb-app")),
