...
import { mkdir, readFile, symlink, access } from "node:fs/promises";
import { homedir, hostname } from "node:os";
import { join, resolve } from "node:path";
import { createServer } from "node:net";
import { mutateManagedJsonFile } from "@bb/config/managed-json-file";
import { readSecretFile, writeSecretFile } from "@bb/secret-storage";
import { z } from "zod";
...
async function readPrivateOptional(path: string): Promise<string | null> {
  return (await readSecretFile(path)) ?? null;
}
...
async function reservePort(dataDir: string): Promise<void> {
  ...
  try {
    ...
    await writeSecretFile(path, `${address.port}\n`);
  } finally {
    if (server.listening)
...
export async function enrollMachine(
  ...
  const existingAuth = await readPrivateOptional(join(dataDir, "auth.json"));
  if (existingAuth !== null) {
    ...
    try {
      ...
      config = configSchema.parse(
        JSON.parse(
          (await readPrivateOptional(join(dataDir, "config.json"))) ?? "{}",
        ),
      );
    } catch {
  ...
  try {
    ...
    const rawAuth = await readPrivateOptional(join(dataDir, "auth.json"));
  ...
  config = await mutateManagedJsonFile({
    ...
    read: async () =>
      configSchema.parse(
        JSON.parse((await readPrivateOptional(configPath)) ?? "{}"),
      ),
    mutate: (current) => {
  ...
  await writeSecretFile(join(dataDir, "host-id"), `${bootstrap.hostId}\n`);
  await prepareRuntime();
