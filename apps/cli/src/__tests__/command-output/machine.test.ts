...
describe("bb machine command output", () => {
  ...
  it("prints the PowerShell command for a Windows manual machine", async () => {
    const windowsCommand = "powershell.exe -NoProfile -File install.ps1";
    stubServerApi({
      "v1.hosts.$post": vi.fn(async () => creating),
      "v1.hosts.:id.enrollment-command.$get": vi.fn(async () => ({
        command: "curl -fsSL https://machine.example/install.sh | sh",
        windowsCommand,
        expiresAt: Date.now() + 60_000,
      })),
      "v1.hosts.:id.$get": vi.fn(async () => hosts[1]),
    ...
    await runCommand(
      ["machine", "create", "--provider", "manual", "--shell", "powershell"],
      register,
    );
    expect(collectLogPayloads(vi.mocked(console.error))).toContain(
      windowsCommand,
    );
    expect(collectLogPayloads(vi.mocked(console.error))).not.toContain(
      "curl -fsSL https://machine.example/install.sh | sh",
    );
  });
  ...
  it.each([
    ["manual", "invalid", []],
    ["ssh", "powershell", []],
    ["manual", "powershell", ["--no-wait"]],
  ])(
    "rejects incompatible shell selection before creating %s",
    async (provider, shell, extra) => {
