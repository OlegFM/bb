...
import { expectPrivateFile } from "../helpers/private-file.js";
...
const UNENCRYPTED_EXPORT_WARNING =
  "This export is not encrypted and holds the server's credentials and plugin secrets. Keep it private; bb restricted access to the current user.";
...
describe("bb server export", () => {
  ...
  it("streams the archive to a private file after checking the server's digest", async () => {
    ...
    const outPath = join(dir, "резервная копия.tar.gz");
    const chunks = [
    ...
    await expectPrivateFile(outPath);
    expect(await readdir(dir)).toEqual(["резервная копия.tar.gz"]);
  });
