...
describePosix("bb server install-machine-service", () => {
  setupCommandOutputTestEnvironment();
...
describePosix("bb server unlock with a machine service", () => {
  setupCommandOutputTestEnvironment();
...
describe.runIf(process.platform === "win32")(
  "bb server install-machine-service on Windows",
  () => {
    setupCommandOutputTestEnvironment();

    it("rejects the unsupported service without stopping bb or launching an installer", async () => {
      const fixture = await createFixture();
      const bb = await startRecordedBb(fixture);
      ...
      expect(collectLogPayloads(vi.mocked(console.error))).toEqual([
        "Error: Installing a background machine service after a server move is not supported on Windows.",
      ]);
      expect(bb.exitCode).toBeNull();
      expect(bb.signalCode).toBeNull();
      await expect(
        readBbAppRuntimeFile(fixture.dataDir),
