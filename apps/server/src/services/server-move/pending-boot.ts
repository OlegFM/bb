...
function rerootPath(
  ...
  if (
    !isHostPathWithin({ rootPath: fromRoot, candidatePath: value }) ||
    (isHostPathWithin({ rootPath: fromRoot, candidatePath: toRoot }) &&
      isHostPathWithin({ rootPath: toRoot, candidatePath: value }))
  ) {
    return null;
  }
  const suffix = normalizeHostPath(value).slice(
    normalizeHostPath(fromRoot).length,
  );
  return joinHostPath(
    toRoot,
    detectHostPathFlavor(fromRoot) === "windows"
      ? suffix.replaceAll("\\", "/")
      : suffix,
  );
}
...
export async function applyServerImportFixups(
  ...
  if (detectHostPathFlavor(fromRoot) === null) {
    throw new Error("Imported server source data path must be absolute");
  }
