...
vi.mock("../machines/git-credentials.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../machines/git-credentials.js")>();
  return {
    ...actual,
    resolveGitCredentials: () =>
      actual.resolveGitCredentials(async (args) =>
        args[0] === "auth"
          ? "test-gh-secret\n"
