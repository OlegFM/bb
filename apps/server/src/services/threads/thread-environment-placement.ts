...
export async function resolveThreadEnvironmentPlacement(
  ...
  switch (resolvedEnvironment.type) {
    ...
    case "host": {
      ...
      const [canonical, canonicalDataDir] = await Promise.all([
        canonicalizeHostPath(deps, {
          hostId: resolvedEnvironment.hostId,
          path: resolvedEnvironment.unmanagedPath,
        }),
        canonicalizeHostDataDir(deps, {
          hostId: resolvedEnvironment.hostId,
          dataDir,
        }),
      ]);
      ...
      return providerPlacement(
        ...
        DEFAULT_ENVIRONMENT_PROVIDER_ID.projectCheckout,
        {
          ...
          inputs: checkoutProviderInputs(canonical.path, workspace.branch),
        },
      );
    }
    case "personal": {
...
async function providerPlacement(
  ...
  if (
    ...
    if (parsed.data !== null) {
      ...
      const { path, pathKey } = await canonicalizeHostPath(deps, {
        hostId,
        path: parsed.data,
      });
