...
it("creates, enrolls, and removes a manual machine by host id", async () => {
  await withTestHarness(async (harness) => {
    ...
    expect(enrollment.windowsCommand).toContain(`/install.ps1`);
    expect(enrollment.windowsCommand).toContain(credential);
    expect(JSON.stringify(getHost(harness.db, created.id))).not.toContain(
      credential,
    );
    const installer = await harness.app.request("/install.ps1", {
      headers: { "X-BB-Enrollment": credential! },
    });
    expect(installer.status).toBe(200);
    const installerBody = await installer.text();
    expect(installerBody).toContain("$BootstrapEnv = 'BB_ENROLLMENT'");
    expect(installerBody).toContain(credential);
    expect(installerBody).toContain("machine', 'enroll'");
    expect(
      (
        await harness.app.request("/install.ps1", {
          headers: { "X-BB-Enrollment": "invalid" },
        })
      ).status,
    ).toBe(403);
