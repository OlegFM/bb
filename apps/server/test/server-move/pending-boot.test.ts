...
describe("imported server boot", () => {
  ...
  it("reroots a Windows registration path with Unicode case and repeated separators", async () => {
    const { dataDir, db, registrationPath } = await openImportedDataDir();
    const sourcePath = "C:\\Users\\Олег\\\\.bb\\plugins\\npm\\tasks";
    db.$client
      .prepare("UPDATE plugins SET root_dir = ? WHERE id = 'tasks'")
      .run(sourcePath);
    await writeFile(
      registrationPath,
      JSON.stringify({ id: "tasks", rootDir: sourcePath }),
    );
    try {
      const result = await applyServerImportFixups({
        dataDir,
        db,
        marker: moveMarker({ sourceDataDir: "c:\\Users\\олег\\.bb" }),
      });
      const expected = join(dataDir, "plugins", "npm", "tasks");
      expect(result.rerooted.plugins).toBe(1);
      expect(result.registrationFiles).toBe(1);
      expect(rootDirOf(db, "tasks")).toBe(expected);
      expect(JSON.parse(await readFile(registrationPath, "utf8"))).toEqual({
        id: "tasks",
        rootDir: expected,
      });
