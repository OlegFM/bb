...
describe.runIf(process.platform !== "win32")("runMachineInstaller", () => {
...
describe("keepMovedMachineConnected", () => {
  it("keeps Windows moves app-connected without looking for or installing a service", async () => {
    const dir = await createTempDir();
    const findService = vi.fn(async () => null);
    const install = vi.fn(async (): Promise<MachineInstallResult> => ({
      ok: true,
    }));
    const stopLocalRuntime = vi.fn(async () => undefined);
    const notices: ServerMovedNotice[] = [];
    const args = {
      findService,
      install,
      ...
      move: MOVE,
      noticeStore: createServerMoveNoticeStore({
        storagePath: join(dir, "machine-service-notice.json"),
      }),
      platform: "win32" as const,
      showNotice: (notice: ServerMovedNotice) => notices.push(notice),
      stopLocalRuntime,
    };

    await expect(keepMovedMachineConnected(args)).resolves.toBe("app-only");
    await expect(keepMovedMachineConnected(args)).resolves.toBe("app-only");
    ...
    expect(notices).toEqual([
      {
        detail: `Keep bb open to keep this computer connected to Studio desktop. A Windows background service was not installed for this move.`,
        message:
