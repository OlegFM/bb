...
export async function keepMovedMachineConnected(
  ...
  if (args.platform === "win32") {
    args.logInfo(
      `[desktop] keeping this computer connected to ${args.move.toHostName} while the app is open; no Windows background service was installed`,
    );
    if (!(await args.noticeStore.hasShown(args.move.moveId))) {
      await args.noticeStore.markShown(args.move.moveId);
      args.showNotice({
        detail: `Keep bb open to keep this computer connected to ${args.move.toHostName}. A Windows background service was not installed for this move.`,
        message: `This computer stays connected to ${args.move.toHostName} only while bb is open`,
      });
    }
    return "app-only";
