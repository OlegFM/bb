...
Manual enrollment offers POSIX and Windows PowerShell commands. Use
`bb machine create --provider manual --shell powershell` for Windows;
`--shell sh` is the default. An explicit shell requires manual creation with
waiting enabled. The SDK's `hosts.experimental_getEnrollmentCommand` returns
`command`, `windowsCommand`, and `expiresAt` for the same private bootstrap.
...
`bb server install-machine-service` on macOS/Linux. Native Windows rejects
this service-adoption command before stopping the local runtime; new Windows
enrollments use the PowerShell installer.
