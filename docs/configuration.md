...
settings. Both are Windows-only: the desktop app and the launcher check the
platform before they read either key, so on macOS and Linux they do nothing at
all. Do not set them in normal use.
