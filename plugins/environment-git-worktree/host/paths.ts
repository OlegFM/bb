...
export function deriveRepoDirName(sourcePath: string): string {
  ...
  const basename = pathApi.basename(sourcePath.replace(trailingSeparators, ""));
...
export function resolveWorktreeChildPath(args: {
  ...
  if (
    ...
    path.win32.basename(args.childName) !== args.childName ||
    !isSafeRepoDirName(args.childName)
  ) {
    throw new WorkspaceError(
      "invalid_source_path",
