...
import { deriveRepoDirName } from "./host/paths.js";
...
describe("worktree host entry", () => {
  ...
  it("creates a bounded worktree path for a long repository name", async () => {
    const oversizedName = "repository".repeat(24);
    expect(
      Buffer.byteLength(
        deriveRepoDirName(join(tmpdir(), oversizedName)),
        "utf8",
      ),
    ).toBe(200);
    const repositoryName = "repository".repeat(
      process.platform === "win32" ? 8 : 24,
    );
    const { sourcePath, dataDir } =
      await createSourceRepository(repositoryName);
    const harness = createHarness(dataDir);
    ...
    expect(Buffer.byteLength(basename(result.path), "utf8")).toBe(
      process.platform === "win32" ? 80 : 200,
    );
