...
describe("automation CLI --script-file", () => {
  ...
  it("resolves the source against ctx.cwd and reports the stored snapshot copy", async () => {
    ...
    try {
      ...
      expect(created.stdout).toContain(
        `bb automation update ${automationId} --project proj_test --script-file ${process.platform === "win32" ? `'${sourcePath}'` : sourcePath} --interpreter bash --working-directory project --timeout 120000`,
      );
  ...
  it("reads the file on the thread's environment host or the --host override", async () => {
    ...
    try {
      ...
      expect(inThread.stdout).toContain(
        `--script-file ${process.platform === "win32" ? `'${sourcePath}'` : sourcePath} --host host_laptop --interpreter bash --working-directory project`,
      );
...
describe("script process containment", () => {
  it.skipIf(process.platform === "win32")(
    "terminates descendant processes when a script times out",
    async () => {
...
describe("script project context", () => {
  ...
  async function runScriptAutomation(args: {
    ...
    await mkdir(scriptDir, { recursive: true });
    await writeFile(join(scriptDir, "script.mjs"), args.script);
    const execution = {
  ...
  it("runs in the actual server-host source instead of a remote primary source", async () => {
    ...
    try {
      const result = await runScriptAutomation({
        script:
          'import { readFileSync, realpathSync } from "node:fs"; console.log(realpathSync(process.cwd())); process.stdout.write(readFileSync("bin/build-views.txt", "utf8"));',
        workingDirectory: { type: "project" },
        sources: [
          projectSource({
            hostId: "host_primary",
  ...
  it("keeps fieldless automations in shared script storage", async () => {
    ...
    try {
      const result = await runScriptAutomation({
        script:
          'import { realpathSync } from "node:fs"; console.log(realpathSync(process.cwd()));',
        workingDirectory: { type: "automation-storage" },
        sources: [
          projectSource({
            hostId: "host_server",
  ...
  it("runs in an explicit absolute server-host directory", async () => {
    ...
    try {
      const result = await runScriptAutomation({
        script:
          'import { readFileSync, realpathSync } from "node:fs"; console.log(realpathSync(process.cwd())); process.stdout.write(readFileSync("selected.txt", "utf8"));',
  ...
  it("includes the first stderr line in a failed run summary", async () => {
    const result = await runScriptAutomation({
      script:
        'process.stdout.write("stdout kept\\n"); process.stderr.write("\\n  missing project file  \\nlater detail\\n"); process.exitCode = 2;',
      workingDirectory: { type: "automation-storage" },
      sources: [],
    });
    expect(result.closed).toMatchObject({
