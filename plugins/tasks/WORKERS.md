...
- CLI against the dev instance: `eval "$(pnpm --silent dev:app env)"` then `pnpm bb:dev tasks <subcommand>`.
...
- If the dev server itself died: `pnpm dev:status` / `pnpm dev:app current`.
