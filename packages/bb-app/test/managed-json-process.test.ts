...
async function seed(dir: string, kind: string, value: object): Promise<string> {
  const path = join(dir, `${kind}.json`);
  await writeSecretFile(path, JSON.stringify(value));
  return path;
}
...
async function expectPrivateFile(filePath: string): Promise<void> {
  if (process.platform === "win32") {
    ...
    const aces = await readSecretFileAcl(filePath);
    expect(() =>
      assertSecretFileAclIsPrivate(filePath, aces, user),
    ).not.toThrow();
  } else {
...
async function cleanAndPrivate(dir: string, kind: string): Promise<void> {
  expect(readdirSync(dir).filter((file) => file.endsWith(".tmp"))).toEqual([]);
  await expectPrivateFile(join(dir, `${kind}.json`));
  if (process.platform !== "win32") {
...
describe("managed JSON CLI process transactions", options, () => {
  for (const { kind, a, b, va, vb } of cases) {
    for (const operation of ["set", "unset"]) {
      for (const pausedOperation of ["set", operation].filter(
        ...
        it(`${kind} ${operation}/set preserves both mutations with ${pausedOperation} paused`, async () => {
          ...
          const path = await seed(dir, kind, {
            ...extras,
          ...
          await cleanAndPrivate(dir, kind);
        });
      }
    }
    ...
    for (const stage of ["before-read", "write", "partial-write", "rename"]) {
      it(`${kind} releases the lock and preserves bytes after ${stage} failure`, async () => {
        const dir = directory();
        const path = await seed(dir, kind, { [kind]: { [a]: va } });
    ...
    for (const stage of ["read", "after-write", "after-rename"]) {
      it(`${kind} recovers after SIGKILL at ${stage} with two waiting writers`, async () => {
        ...
        if (stage === "after-write")
          await expectPrivateFile(join(dir, `.${kind}.json.tmp`));
  ...
  it("same-process contenders preserve the kernel lock seen by other CLI processes", async () => {
    const dir = directory();
    const path = await seed(dir, "env", { env: { SYNTHETIC: "keep" } });
    let release!: () => void;
    let acquired!: () => void;
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const dir = directory();
      const path = await seed(dir, "env", {
        env: { SYNTHETIC_A: "synthetic" },
      });
  ...
  it("rejects invalid input and malformed documents without rewriting them", async () => {
    const dir = directory();
    const path = await seed(dir, "config", {
  ...
  it("cleans crash leftovers without reusing their permissions or contents", async () => {
    const dir = directory();
    const path = await seed(dir, "env", { env: { SYNTHETIC_A: "synthetic" } });
  ...
  for (const remove of [false, true]) {
    it(`client mappings serialize set/${remove ? "remove" : "set"}`, async () => {
      ...
      const path = await seed(dir, "client", {
        servers: {
      ...
      await cleanAndPrivate(dir, "client");
    });
  }
  ...
  it("serializes machine enrollment with launcher config changes", async () => {
    ...
    await cleanAndPrivate(dir, "config");
  });
});
