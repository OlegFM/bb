...
The generated command retrieves a private bootstrap bundle using the
`X-BB-Enrollment` request header; the installer consumes it through
`bb machine enroll --bootstrap-env`. An optional `-HostDaemonPort` selects the
local daemon port. It requires the server's SHA-256-verified host artifact;
...
For CLI pairing, run `bb machine create --provider manual --shell powershell`
and execute its printed command on Windows before it expires. Omit `--shell`
for a POSIX target. An explicit shell requires the manual provider and cannot
be combined with `--no-wait`. The SDK equivalent is
`sdk.hosts.experimental_create({ machineProviderId: "manual", inputs: null, wait: false })`,
then poll `sdk.hosts.experimental_getEnrollmentCommand({ hostId })` until it
returns a command. Select `windowsCommand` for PowerShell or `command` for sh;
both share `expiresAt`. The server supplies access credentials in the private
bootstrap bundle. Configure bb Connect or a reachable direct server URL before
...
the same service on macOS/Linux; it is unavailable on native Windows. It needs
