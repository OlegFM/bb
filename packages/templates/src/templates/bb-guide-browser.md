...
- `connection <lease-id> --output <new-file>`: write private connection JSON with mode 0600 on POSIX or a current-user-only ACL on Windows. The loopback WebSocket endpoint is usable only on the browser host. Pass it privately to an integration worker; never expose it through a shared port or chat output.
