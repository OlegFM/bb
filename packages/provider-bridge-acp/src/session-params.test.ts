  ...
  it("appends sanitized skill instructions after the base instructions", () => {
    expect(
      ...
      instructions: [
        ...
        "",
        "Available bb skills:",
        `- release-notes: Use release-notes when /system_instructions tests run. (SKILL.md: ${join("/tmp/bb/runtime/global-skills/abc123/skills", "release-notes", "SKILL.md")})`,
        `- copywriting: Use when writing customer copy. (SKILL.md: ${join("/tmp/bb/runtime/global-skills/abc123/skills", "copywriting", "SKILL.md")})`,
      ].join("\n"),
    });
  });
  ...
  it("starts with the skill block when the session has no base instructions", () => {
    expect(
      ...
      instructions: [
        ...
        "",
        "Available bb skills:",
        `- debugging: Use when debugging runtime state. (SKILL.md: ${join("/tmp/bb/runtime/global-skills/def456/skills", "debugging", "SKILL.md")})`,
      ].join("\n"),
    });
  });
