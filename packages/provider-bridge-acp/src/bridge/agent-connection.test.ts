...
describe("ACP agent stdio lifecycle", () => {
  it("does not surface a closed agent stdin as an unhandled error", async () => {
  ...
  it("rejects pending requests and reaps the agent after a Windows stdin EOF", async () => {
    const child = spawn(
      process.execPath,
      ["-e", "setInterval(() => {}, 1000)"],
      {
    ...
    try {
      ...
      ).rejects.toThrow("stdin closed (EOF)");
      child.stdin.emit(
        "error",
  ...
  it.skipIf(process.platform === "win32")(
    "ignores an ACP request emitted during SIGTERM",
    async () => {
