...
export function sharedDeclarationEmit(
  entries,
  workspaceDir,
  resolveSource,
  ...
  const roots = [
    ...new Set(
      discovery
        ...
        .filter((id) => {
          const relativePath = path.relative(resolvedWorkspaceDir, id);
          return (
            relativePath !== "" &&
            relativePath !== ".." &&
  ...
  function programFor(file) {
    ...
    if (
      ...
      !previous
        .getRootFileNames()
        .some((root) => path.resolve(root) === path.resolve(file))
    ) {
      programs.set(
        config.key,
        createProgram(
