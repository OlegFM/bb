...
it("reuses one real compiler program for all discovered workspace sources", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "bb-shared-roots-"));
  try {
    ...
    await writeFile(entry, 'export { value } from "./dependency";');
    await writeFile(dependency, "export const value = { count: 1 } as const;");
    const programRoots = [];
    const createProgram = (...args) => {
      programRoots.push(args[0]);
      return ts.createProgram(...args);
    };
    const shared = sharedDeclarationEmit(
      [entry],
      root,
      () => null,
      createProgram,
    ...
    expect(shared.load(declarationId(dependency))).toContain(
      "readonly count: 1",
    );
    expect(programRoots).toHaveLength(2);
    expect(programRoots[1].map((file) => path.resolve(file))).toEqual(
      expect.arrayContaining([entry, dependency]),
    );
  } finally {
    await rm(root, { recursive: true, force: true });
  }
