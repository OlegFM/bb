...
function isUnderRoot(column: SQLiteColumn, root: string): SQL {
  const windows = detectHostPathFlavor(root) === "windows";
  const comparable = windows
    ? sql`bb_host_path_key(${column})`
    : sql`${column}`;
  const rootKey = windows ? buildHostPathKey(root) : root;
  const prefix = rootKey.endsWith("/") ? rootKey : `${rootKey}/`;
  return or(
    sql`${comparable} = ${rootKey}`,
    sql`substr(${comparable}, 1, length(${prefix})) = ${prefix}`,
  )!;
}

function rerootedValue(column: SQLiteColumn, args: RerootServerOwnedPathsArgs) {
  const suffix = sql`substr(bb_normalize_host_path(${column}), length(${normalizeHostPath(args.fromRoot)}) + 1)`;
  const sourceSuffix =
    detectHostPathFlavor(args.fromRoot) === "windows"
      ? sql`replace(${suffix}, ${"\\"}, ${"/"})`
      : suffix;
  const targetSeparator =
    detectHostPathFlavor(args.toRoot) === "windows" ? "\\" : "/";
  return sql`${args.toRoot} || replace(${sourceSuffix}, ${"/"}, ${targetSeparator})`;
}
...
function rerootCondition(
  ...
  const toRootNestedInFromRoot = isHostPathWithin({
    rootPath: args.fromRoot,
    candidatePath: args.toRoot,
  });
