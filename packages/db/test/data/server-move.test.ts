...
describe("server move data helpers", () => {
  ...
  it("re-roots imported POSIX plugin paths to native Windows paths", () => {
    seedPluginPaths(db);
    ...
    expect(pluginPaths(db).find((plugin) => plugin.id === "server-owned")).toMatchObject({
      rootDir: `${targetRoot}\\plugins\\local-copy`,
      sourcePath: `${targetRoot}\\plugins\\local-copy`,
    });
    expect(listPluginArtifacts(db, "server-owned")[0]?.path).toBe(
      `${targetRoot}\\plugins\\cache\\git\\abc123\\plugin`,
    );
    expect(
      rerootServerOwnedPluginPaths(db, {
        fromRoot: SOURCE_ROOT,
  ...
  it("matches Windows source paths across case and separator differences", () => {
    upsertInstalledPlugin(
      db,
      pathPlugin(
        "windows-source",
        "c:/Users/old/.bb/plugins/local",
        "c:/Users/old/.bb/plugins/local",
      ),
    );
    ...
    expect(
      rerootServerOwnedPluginPaths(db, {
        fromRoot: "C:\\Users\\Old\\.bb",
        toRoot: "/srv/bb",
      }),
    ).toMatchObject({ plugins: 2 });
    expect(pluginPaths(db)[0]).toMatchObject({
      rootDir: "/srv/bb/plugins/local",
      sourcePath: "/srv/bb/plugins/local",
    });
  });

  it("matches Unicode Windows source paths using canonical case folding", () => {
    upsertInstalledPlugin(
      db,
      pathPlugin(
        "unicode-windows-source",
        "c:/Users/Олег//.bb/plugins/local",
        "c:/Users/Олег//.bb/plugins/local",
      ),
    );
