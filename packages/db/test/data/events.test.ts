...
describe("stored provider thread identity ownership", () => {
  function setupThreads(args: { providerIds?: readonly string[] } = {}) {
    ...
    const { project } = createProject(db, noopNotifier, {
      ...
      source: { type: "local_path", hostId: host.id, path: "/tmp/identity", pathKey: "/tmp/identity" },
    });
    const environment = createEnvironment(db, noopNotifier, {
      projectId: project.id,
      hostId: host.id,
      path: "/tmp/identity",
      pathKey: "/tmp/identity",
      status: "ready",
      providerOwnsPath: false,
    });
